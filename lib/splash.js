...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    dom('.holder',
      ...
      dom('p',
        'Присъедини се към чат канала на ', dom('br'), dom('b', 'VarnaLab'),
      ...
      dom('p.status',
        active
          ? [
            dom('b.active', active), ' потребители са активни в момента от ',
            dom('br'),
            dom('b.total', total), ' регистрирани'
          ]
          : [dom('b.total', total), ' регистрирани потребители до момента']
      ),
      dom('form id=invite',
        channels && (
          channels.length > 1
        ...
        dom('button.loading', 'Вземи своята покана сега!')
      ),
      dom('p.signin',
        'или ',
        dom(`a href=https://${org}.slack.com target=_top`, 'се логни')
      )
    ),
    dom('.overlay'),
